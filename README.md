...
- **画像からのレシピ生成（レシピ保管庫）**：冷蔵庫の食材や調理済みの料理の写真をアップロードすると、AIがマルチモーダルビジョン（Gemini Vision）を活用して食材を自動認識し、健康・減量向けのレシピ（視覚的な分量目安や簡易的な栄養分析付き）を生成。ワンクリックで「レシピ保管庫」に保存していつでも閲覧可能です。
...
- **ワンインワン起動設計**：
  ...
  - **自動定期実行（バックグラウンド）**：Web サーバーを立ち上げるだけで、毎朝 `07:00` に今日のメニューを自動生成し、Markdown 形式で保存します。
